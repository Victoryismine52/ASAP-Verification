...
<div id='demotour' class='tab'><div class='card'><h2>Live Verification Demo</h2><p class='subtitle'>Load records, choose a verification source, run eligibility checks, persist results, stage outbox records, and export downstream files.</p><p class='factory-note'>Live Availity data requires vendor access. Mock mode uses the same workflow with demo data.</p><div class='controls'><label for='demo-provider'>Verification Source</label><select id='demo-provider'></select><button class='btn btn-secondary' onclick='setLiveDemoProvider()'>Set Source</button><button class='btn btn-danger' onclick='resetLiveDemo()'>Reset Demo Data</button><button class='btn btn-primary' onclick='loadLiveDemoPatients()'>Load Demo Patients</button><button class='btn btn-secondary' onclick='refreshLiveDemo()'>Refresh</button><button class='btn btn-primary' onclick='validateLiveDemoSelected()'>Validate Selected</button><button class='btn btn-primary' onclick='validateLiveDemoAllPending()'>Validate All Pending</button><a class='link-btn btn btn-secondary' href='/work-items/export.csv'>Download Results CSV</a></div><div id='live-demo-metrics' class='metrics'></div><div class='table-wrap'><table id='live-demo-table' class='data-table'></table></div><h3>Activity Log</h3><div id='demo-activity' class='activity-feed'></div><div class='controls'><button class='btn btn-secondary btn-sm' onclick="showTab('workqueue')">Open Work Queue</button><button class='btn btn-secondary btn-sm' onclick="showTab('history')">Open Request History</button><button class='btn btn-secondary btn-sm' onclick="showTab('outbox')">Open Outbox</button><button class='btn btn-secondary btn-sm' onclick="showTab('manual')">Open Manual Test</button><button class='btn btn-secondary btn-sm' onclick="showTab('providers')">Open Providers</button></div></div></div>
...
async function loadLiveDemoProviders(){const meta=await fetch('/ui/connections').then(r=>r.json());const sel=document.getElementById('demo-provider');if(!sel)return;sel.innerHTML='';for(const p of meta.providers){const o=document.createElement('option');o.value=p;o.textContent=p;if(p===meta.current_provider)o.selected=true;sel.appendChild(o);}}
async function setLiveDemoProvider(){const provider=document.getElementById('demo-provider').value;await fetch('/ui/select-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})});addLiveDemoActivity(`POST /ui/select-connection → provider set to ${provider}`);}
...
async function validateLiveDemoSelected(){const ids=selectedLiveDemoIds();const provider=document.getElementById('demo-provider').value;const r=await fetch('/work-items/validate-selected',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids,provider})}).then(x=>x.json());addLiveDemoActivity(`POST /work-items/validate-selected → validated ${r.validated_count}, failed ${r.failed_count}`);await refreshLiveDemo();await loadWork();await loadHistory();await loadOutbox();await loadDemoCounts();await dashboard();}
async function validateLiveDemoAllPending(){const provider=document.getElementById('demo-provider').value;await fetch('/ui/select-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})});addLiveDemoActivity(`POST /ui/select-connection → provider set to ${provider}`);const r=await fetch('/work-items/validate-pending',{method:'POST'}).then(x=>x.json());addLiveDemoActivity(`POST /work-items/validate-pending → validated ${r.validated_count}`);await refreshLiveDemo();await loadWork();await loadHistory();await loadOutbox();await loadDemoCounts();await dashboard();}
