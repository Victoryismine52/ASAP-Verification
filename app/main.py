...
<div id='patientconsole' class='tab'><div class='card'><h2>Patient Console</h2><div class='card banner'>Use this console to load or create a patient verification record, adjust values, choose a verification source, and run an available service. Today the live service action is eligibility_check; future actions can include benefits detail, prior authorization, claim status, and NextGen writeback.</div><div class='controls'><select id='pc-record-select'></select><button class='btn btn-secondary' onclick='loadSelectedPatientRecord()'>Load Selected Record</button><button class='btn btn-secondary' onclick='createNewPatientRecord()'>Create New Record</button></div><div class='controls'><input id='pc-first_name' placeholder='first_name'/><input id='pc-last_name' placeholder='last_name'/><input id='pc-dob' placeholder='dob (YYYY-MM-DD)'/><input id='pc-member_id' placeholder='member_id'/></div><div class='controls'><input id='pc-payer_name' placeholder='payer_name'/><input id='pc-payer_id' placeholder='payer_id'/><input id='pc-npi' placeholder='npi'/><input id='pc-tax_id' placeholder='tax_id'/></div><div class='controls'><input id='pc-service_type' placeholder='service_type'/><input id='pc-notes' placeholder='notes' style='min-width:240px'/><input id='pc-manual_override_reason' placeholder='manual_override_reason' style='min-width:260px'/></div><div class='controls'><label for='pc-provider'>Provider</label><select id='pc-provider'></select><label for='pc-action'>Service Action</label><select id='pc-action'><option value='eligibility_check'>eligibility_check</option></select></div><div class='controls'><button class='btn btn-primary' onclick='savePatientConsoleRecord()'>Save Record</button><button class='btn btn-primary' onclick='saveAndValidatePatientConsoleRecord()'>Save + Validate</button><button class='btn btn-primary' onclick='runSelectedService()'>Run Selected Service</button><button class='btn btn-secondary' onclick='clearPatientConsoleForm()'>Clear Form</button></div><pre id='pc-response'></pre></div></div>
<div id='demotour' class='tab'><div class='card'><h2>API Factory Demo</h2><p class='subtitle'>Watch records move through an API service pipeline: load records, make JSON, send to an endpoint, receive a response, save the result, and stage the output.</p><div class='timeline'><span class='stage-pill'>Load patients</span><span class='stage-pill'>Make JSON</span><span class='stage-pill'>Send to API</span><span class='stage-pill'>Get answer</span><span class='stage-pill'>Save result</span><span class='stage-pill'>Stage output</span></div><p class='factory-note'>Live Availity data requires vendor access. Mock mode uses the same workflow with demo data.</p><div class='controls'><label for='factory-provider'>Verification Source</label><select id='factory-provider'></select><button class='btn btn-secondary' onclick='setLiveDemoProvider()'>Set Source</button><button class='btn btn-danger' onclick='resetLiveDemo()'>Reset Demo Data</button><button class='btn btn-primary' onclick='loadLiveDemoPatients()'>Load Unknown Demo Patients</button><button class='btn btn-secondary' onclick='refreshLiveDemo()'>Refresh</button><button class='btn btn-secondary' onclick='previewLiveDemoRequest()'>Preview Request JSON</button><button class='btn btn-primary' onclick='validateLiveDemoSelected()'>Send Selected</button><button class='btn btn-primary' onclick='validateLiveDemoAllPending()'>Send All Pending</button><a class='link-btn btn btn-secondary' href='/work-items/export.csv'>Export Results CSV</a></div><pre id='live-demo-preview'></pre><div id='live-demo-metrics' class='metrics'></div><div class='table-wrap'><table id='live-demo-table' class='data-table'></table></div><h3>Activity Log</h3><div id='demo-activity' class='activity-feed'></div><div class='controls'><button class='btn btn-secondary btn-sm' onclick="showTab('workqueue')">Open Work Queue</button><button class='btn btn-secondary btn-sm' onclick="showTab('history')">Open Request History</button><button class='btn btn-secondary btn-sm' onclick="showTab('outbox')">Open Outbox</button><button class='btn btn-secondary btn-sm' onclick="showTab('manual')">Open Manual Test</button><button class='btn btn-secondary btn-sm' onclick="showTab('providers')">Open Providers</button></div></div></div>
...
window.showTab = function(id) {const target = document.getElementById(id);if (!target) return;document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));document.querySelectorAll('.tabs button').forEach(b => {b.classList.toggle('active', b.dataset.tab === id);});target.classList.add('active');if(id==='providers'){loadProviders();}};
...
function selectedLiveDemoIds(){return Array.from(document.querySelectorAll('.live-demo-select:checked')).map(x=>Number(x.value));}
function renderLiveDemoTable(){const t=document.getElementById('live-demo-table');if(!t)return;let html="<thead><tr><th><input type=\"checkbox\" id=\"live-demo-all\"></th><th>ID</th><th>Patient</th><th>DOB</th><th>Member ID</th><th>Payer</th><th>Service</th><th>Status</th><th>Last Checked</th></tr></thead><tbody>";for(const i of liveDemoItems){html+=`<tr><td><input class='live-demo-select' type='checkbox' value='${i.id}'></td><td>${i.id}</td><td>${i.first_name||''} ${i.last_name||''}</td><td>${i.dob||''}</td><td>${i.member_id||''}</td><td>${i.payer_name||''}</td><td>${i.service_type||''}</td><td>${badge(i.validation_status)}</td><td>${i.last_validated_at||''}</td></tr>`;}t.innerHTML=html+'</tbody>';const selectAll=document.getElementById('live-demo-all');if(selectAll){selectAll.addEventListener('change',(event)=>{document.querySelectorAll('.live-demo-select').forEach((el)=>{el.checked=event.target.checked;});});}}
...
async function validateLiveDemoSelected(){const ids=selectedLiveDemoIds();const provider=document.getElementById('factory-provider').value;const r=await fetch('/work-items/validate-selected',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids,provider})}).then(x=>x.json());addLiveDemoActivity(`POST /work-items/validate-selected → validated ${r.validated_count}, failed ${r.failed_count}`);await refreshLiveDemo();await loadWork();await loadHistory();await loadOutbox();await loadDemoCounts();await dashboard();}
async function previewLiveDemoRequest(){const ids=selectedLiveDemoIds();if(ids.length!==1){alert('Select exactly one row');return;}const r=await fetch(`/work-items/${ids[0]}/preview-request`,{method:'POST'}).then(x=>x.json());document.getElementById('live-demo-preview').textContent=JSON.stringify(r,null,2);addLiveDemoActivity(`POST /work-items/${ids[0]}/preview-request → built JSON payload`);}
async function validateLiveDemoAllPending(){const provider=document.getElementById('factory-provider').value;await fetch('/ui/select-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})});addLiveDemoActivity(`POST /ui/select-connection → provider set to ${provider}`);const pendingIds=liveDemoItems.filter(i=>i.needs_validation).map(i=>i.id);const r=await fetch('/work-items/validate-selected',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids:pendingIds,provider})}).then(x=>x.json());addLiveDemoActivity(`POST /work-items/validate-selected → validated ${r.validated_count}, failed ${r.failed_count}`);await refreshLiveDemo();await loadWork();await loadHistory();await loadOutbox();await loadDemoCounts();await dashboard();}
...
let activePatientConsoleId=null;
function patientConsolePayload(){return {first_name:document.getElementById('pc-first_name').value,last_name:document.getElementById('pc-last_name').value,dob:document.getElementById('pc-dob').value,member_id:document.getElementById('pc-member_id').value,payer_name:document.getElementById('pc-payer_name').value,payer_id:document.getElementById('pc-payer_id').value,npi:document.getElementById('pc-npi').value,tax_id:document.getElementById('pc-tax_id').value,service_type:document.getElementById('pc-service_type').value||'30',notes:document.getElementById('pc-notes').value,manual_override_reason:document.getElementById('pc-manual_override_reason').value};}
function populatePatientConsoleForm(i){activePatientConsoleId=i.id||null;document.getElementById('pc-first_name').value=i.first_name||'';document.getElementById('pc-last_name').value=i.last_name||'';document.getElementById('pc-dob').value=i.dob||'';document.getElementById('pc-member_id').value=i.member_id||'';document.getElementById('pc-payer_name').value=i.payer_name||'';document.getElementById('pc-payer_id').value=i.payer_id||'';document.getElementById('pc-npi').value=i.npi||'';document.getElementById('pc-tax_id').value=i.tax_id||'';document.getElementById('pc-service_type').value=i.service_type||'30';document.getElementById('pc-notes').value=i.notes||'';document.getElementById('pc-manual_override_reason').value=i.manual_override_reason||'';}
async function loadPatientConsoleRecords(){const w=await fetch('/work-items').then(r=>r.json());const sel=document.getElementById('pc-record-select');sel.innerHTML='<option value="">Select work item...</option>';for(const i of w.items){const o=document.createElement('option');o.value=i.id;o.textContent=`#${i.id} | ${i.first_name||''} ${i.last_name||''} | ${i.payer_name||''} | ${i.validation_status||''}`;sel.appendChild(o);}const meta=await fetch('/ui/connections').then(r=>r.json());const psel=document.getElementById('pc-provider');psel.innerHTML='';for(const p of meta.providers){const o=document.createElement('option');o.value=p;o.textContent=p;if(p===meta.current_provider)o.selected=true;psel.appendChild(o);}}
async function loadSelectedPatientRecord(){const id=document.getElementById('pc-record-select').value;if(!id)return;const i=await fetch(`/work-items/${id}`).then(r=>r.json());populatePatientConsoleForm(i);}
function createNewPatientRecord(){activePatientConsoleId=null;clearPatientConsoleForm();document.getElementById('pc-service_type').value='30';}
async function savePatientConsoleRecord(){const payload=patientConsolePayload();let r;if(activePatientConsoleId){r=await fetch(`/work-items/${activePatientConsoleId}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});}else{r=await fetch('/work-items',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});}const data=await r.json();if(data.id){activePatientConsoleId=data.id;}document.getElementById('pc-response').textContent=JSON.stringify(data,null,2);await loadWork();await loadPatientConsoleRecords();return activePatientConsoleId;}
async function saveAndValidatePatientConsoleRecord(){const id=await savePatientConsoleRecord();if(!id)return;const vr=await fetch(`/work-items/${id}/validate`,{method:'POST'});document.getElementById('pc-response').textContent=JSON.stringify(await vr.json(),null,2);await loadWork();await loadHistory();await loadOutbox();await dashboard();}
async function runSelectedService(){const provider=document.getElementById('pc-provider').value;await fetch('/ui/select-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})});const p=patientConsolePayload();const payload={patient:{first_name:p.first_name,last_name:p.last_name,dob:p.dob,member_id:p.member_id},payer:{name:p.payer_name,payer_id:p.payer_id},provider:{npi:p.npi,tax_id:p.tax_id},service_type:p.service_type||'30'};const r=await fetch('/ui/test-call',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});document.getElementById('pc-response').textContent=JSON.stringify(await r.json(),null,2);await loadHistory();await loadOutbox();await dashboard();}
function clearPatientConsoleForm(){activePatientConsoleId=null;for(const id of ['pc-first_name','pc-last_name','pc-dob','pc-member_id','pc-payer_name','pc-payer_id','pc-npi','pc-tax_id','pc-service_type','pc-notes','pc-manual_override_reason']){document.getElementById(id).value='';}document.getElementById('pc-response').textContent='';}
...
(async()=>{document.getElementById('payload').value=JSON.stringify(defaultManualPayload,null,2);populateFormFromPayload(defaultManualPayload);setManualMode('form');showTab('dashboard');addLiveDemoActivity('Live demo ready.');await dashboard();await loadWork();await loadHistory();await loadOutbox();await loadDemoCounts();await loadProviders();await loadManualProviders();await loadLiveDemoProviders();await refreshLiveDemo();await loadPatientConsoleRecords();createNewPatientRecord();})();
</script></body></html>"""
