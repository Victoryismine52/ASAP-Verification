...
async function validateLiveDemoOne(id) {const provider = document.getElementById('factory-provider').value;const r = await fetch('/work-items/validate-selected', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ ids: [id], provider })}).then(x => x.json());addLiveDemoActivity(`POST /work-items/validate-selected → validated ${r.validated_count}, failed ${r.failed_count}`);await refreshLiveDemo();await loadWork();await loadHistory();await loadOutbox();await loadDemoCounts();await dashboard();}
