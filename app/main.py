...
async function setLiveDemoProvider(){const provider=document.getElementById('factory-provider').value;await fetch('/ui/select-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})});addLiveDemoActivity(`POST /ui/select-connection → provider set to ${provider}`);await renderFactorySourceStatus();}
async function refreshLiveDemo(){const [w,h,o]=await Promise.all([fetch('/work-items').then(r=>r.json()),fetch('/history').then(r=>r.json()),fetch('/outbox').then(r=>r.json())]);liveDemoItems=w.items||[];liveDemoHistory=h.items||[];liveDemoOutbox=o.items||[];renderLiveDemoTable();renderLiveDemoMetrics();const refreshTime=document.getElementById('factory-refresh-time');if(refreshTime){refreshTime.textContent=`Refreshed at ${new Date().toLocaleString()}`;}await renderFactorySourceStatus();}
