...
<div id='providers' class='tab'><h2>Providers</h2><h3>Provider Adapter Matrix</h3><label for='provider'>Active Provider: </label><select id='provider'></select><button id='switch-btn'>Switch</button><button onclick='loadProviders()'>Refresh Provider Status</button><pre id='provider-status'></pre><pre id='details'></pre><div id='adapter-matrix'></div></div>
...
<script>
function showTab(id){document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));document.getElementById(id).classList.add('active');if(id==='providers'){loadProviders();}}
