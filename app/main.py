...
def parse_stored_json(value: str | None):
    if not value:
        return None
    try:
...
ACTIVE_ELIGIBILITY_STATUSES = {"active", "validated", "complete_active_coverage", "complete_active", "active_coverage"}
REVIEW_ELIGIBILITY_STATUSES = {"inactive", "not_active", "unknown", "error", "ambiguous", "payer_error", "requires_review"}


def work_item_status_from_response(response) -> tuple[str, bool, str | None]:
    """Map provider response content to work-item validation state."""
    if response.error_message:
        return "ready_for_review", True, response.error_message
    normalized_status = (response.status or "unknown").strip().lower()
    if normalized_status in ACTIVE_ELIGIBILITY_STATUSES:
        return "validated", False, None
    if normalized_status in REVIEW_ELIGIBILITY_STATUSES:
        return "ready_for_review", True, f"Provider response requires review: {response.status}"
    return "ready_for_review", True, f"Provider response requires review: {response.status or 'unknown'}"
...
.factory-note{font-size:.85rem;color:#475569;background:#f8fafc;border:1px dashed #cbd5e1;padding:.55rem;border-radius:10px}.latest-action{margin:.6rem 0;padding:.55rem .65rem;border:1px solid #cbd5e1;border-radius:10px;background:#f8fafc;font-size:.85rem}.modal-backdrop{position:fixed;inset:0;background:rgba(15,23,42,.45);display:none;align-items:center;justify-content:center;z-index:2000}.modal-backdrop.open{display:flex}.modal{background:#fff;width:min(960px,92vw);max-height:80vh;border-radius:12px;border:1px solid var(--border);display:flex;flex-direction:column}.modal-header{padding:.75rem 1rem;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center}.modal-body{padding:.85rem 1rem;overflow:auto;max-height:calc(80vh - 120px)}.modal-actions{padding:.75rem 1rem;border-top:1px solid var(--border);display:flex;gap:.5rem;justify-content:flex-end}.scroll-json{max-height:220px;overflow:auto;border:1px solid #e5e7eb;border-radius:8px;background:#f8fafc;padding:.5rem;white-space:pre-wrap;word-break:break-word}
...
<div id='admin' class='tab'><div class='card'><h2>Admin</h2><p class='subtitle'>Provider Connection Settings for local/demo runtime overrides. Values are stored in memory only and reset when the app restarts. Secret values are masked; leave secret inputs blank to keep the current secret unchanged.</p><div class='card banner'><strong>Runtime-only:</strong> this panel does not write .env files and API responses never return full secrets.</div><h3>Provider Connection Settings</h3><div class='form-grid'><div class='field'><label for='admin-eligibility-provider'>ELIGIBILITY_PROVIDER</label><input id='admin-eligibility-provider' /></div><div class='field'><label for='admin-database-url'>DATABASE_URL (view-only)</label><input id='admin-database-url' readonly /></div></div><h3>Stedi</h3><div class='form-grid'><div class='field'><label for='admin-stedi-api-key'>STEDI_API_KEY</label><input id='admin-stedi-api-key' type='password' placeholder='Leave blank to keep current masked secret' /></div><div class='field'><label for='admin-stedi-base-url'>STEDI_BASE_URL</label><input id='admin-stedi-base-url' /></div><div class='field'><label for='admin-stedi-provider-organization-name'>STEDI_PROVIDER_ORGANIZATION_NAME</label><input id='admin-stedi-provider-organization-name' /></div></div><h3>Availity</h3><div class='form-grid'><div class='field'><label for='admin-availity-client-id'>AVAILITY_CLIENT_ID</label><input id='admin-availity-client-id' /></div><div class='field'><label for='admin-availity-client-secret'>AVAILITY_CLIENT_SECRET</label><input id='admin-availity-client-secret' type='password' placeholder='Leave blank to keep current masked secret' /></div><div class='field'><label for='admin-availity-base-url'>AVAILITY_BASE_URL</label><input id='admin-availity-base-url' /></div><div class='field'><label for='admin-availity-scope'>AVAILITY_SCOPE</label><input id='admin-availity-scope' /></div><div class='field'><label for='admin-availity-submitter-id'>AVAILITY_SUBMITTER_ID</label><input id='admin-availity-submitter-id' /></div><div class='field'><label for='admin-availity-provider-npi'>AVAILITY_PROVIDER_NPI</label><input id='admin-availity-provider-npi' /></div><div class='field'><label for='admin-availity-provider-tax-id'>AVAILITY_PROVIDER_TAX_ID</label><input id='admin-availity-provider-tax-id' /></div></div><div class='controls'><button class='btn btn-primary' onclick='saveAdminConfig()'>Save Runtime Settings</button><button class='btn btn-secondary' onclick='adminSetSource()'>Test Connection / Set Source</button><button class='btn btn-danger' onclick='resetAdminConfig()'>Reset Runtime Overrides</button><button class='btn btn-secondary' onclick='loadAdminConfig()'>Refresh</button></div><pre id='admin-status'></pre></div></div>
	<div id='manual' class='tab'><div class='card'><h2>Manual Test</h2><p class='subtitle'>Use this tab to manually create or edit an eligibility request, choose a verification source, and run the selected service. Form view is for operational testing; raw JSON is for developer/API testing.</p><div class='controls'><label for='manual-provider'>Verification Source</label><select id='manual-provider'></select><label for='manual-action'>Service Action</label><select id='manual-action'><option value='eligibility_check'>eligibility_check</option></select><button class='btn btn-secondary' type='button' onclick="setManualMode('form')">Form View</button><button class='btn btn-secondary' type='button' onclick="setManualMode('json')">Raw JSON</button></div><div id='manual-form-card' class='card'><div class='form-grid'><div class='field'><label for='first_name'>first_name</label><input id='first_name' /></div><div class='field'><label for='last_name'>last_name</label><input id='last_name' /></div><div class='field'><label for='dob'>dob</label><input id='dob' /></div><div class='field'><label for='member_id'>member_id</label><input id='member_id' /></div><div class='field'><label for='external_patient_id'>external_patient_id</label><input id='external_patient_id' /></div><div class='field'><label for='payer_name'>payer_name</label><input id='payer_name' /></div><div class='field'><label for='payer_id'>payer_id</label><input id='payer_id' /></div><div class='field'><label for='npi'>npi</label><input id='npi' /></div><div class='field'><label for='tax_id'>tax_id</label><input id='tax_id' /></div><div class='field'><label for='service_type'>service_type</label><input id='service_type' /></div></div></div><div id='manual-json-card' class='card hidden'><textarea id='payload' rows='12'></textarea><div class='controls'><button class='btn btn-secondary' type='button' onclick='formatManualJson()'>Format JSON</button><button class='btn btn-secondary' type='button' onclick='syncJsonToForm()'>Sync JSON to Form</button><button class='btn btn-secondary' type='button' onclick='syncFormToJson()'>Sync Form to JSON</button></div></div><div class='controls'><button class='btn btn-primary' id='test-btn'>Run Test Call</button></div><pre id='test-result'></pre></div></div>
...
<div id='response-modal-backdrop' class='modal-backdrop'><div class='modal'><div class='modal-header'><h3>API Response Payload</h3><button class='btn btn-secondary btn-sm' onclick='closeResponseModal()'>Close</button></div><div id='response-modal-body' class='modal-body'></div><div class='modal-actions'><button class='btn btn-secondary' onclick='closeResponseModal()'>Close</button><button class='btn btn-secondary' onclick="showTab('history')">Open Request History</button><button class='btn btn-secondary' onclick="showTab('outbox')">Open Outbox</button></div></div></div></main></div>
<script>
function badge(s){return `<span class="status-badge ${s||''}">${s||''}</span>`;}
function reviewText(i){if(!i?.last_error_message)return '';const prefix=i.validation_status==='ready_for_review'?'API responded; eligibility requires review: ':'';return `${prefix}${i.last_error_message}`;}
function shortId(value){if(!value)return '';const v=String(value);return v.length>14?`${v.slice(0,14)}…`:v;}
function checkTypeLabel(value) {if (!value) return "Eligibility / Coverage";if (String(value) === "30") return "Health Benefit Plan Coverage (30)";return `Eligibility / Coverage (${value})`;}
...
async function fetchHistoryDetail(requestId){if(!requestId)return null;return await fetch(`/history/${requestId}`).then(r=>r.json());}
function renderJsonBlock(v){const value=v===undefined?null:v;return `<pre class='scroll-json'>${JSON.stringify(value,null,2)}</pre>`;}
function openResponseModal(detail){const backdrop=document.getElementById('response-modal-backdrop');const body=document.getElementById('response-modal-body');if(!backdrop||!body)return;body.innerHTML=`<p><strong>Provider:</strong> ${detail?.request?.provider_source||detail?.provider||''}</p><p><strong>Request ID:</strong> ${detail?.request?.request_id||detail?.request_id||''}</p><p><strong>Patient:</strong> ${(detail?.request_payload?.patient?.first_name||'')} ${(detail?.request_payload?.patient?.last_name||'')} (${detail?.request_payload?.patient?.member_id||''})</p><p><strong>Status:</strong> ${detail?.request?.status||detail?.result?.eligibility_status||''}</p><h4>Request Payload</h4>${renderJsonBlock(detail?.request_payload)}<h4>Response Payload</h4>${renderJsonBlock(detail?.result?.raw_response_json)}<h4>Result Summary</h4>${renderJsonBlock(detail?.result)}<h4>Outbox Status</h4>${renderJsonBlock(detail?.outbox||{})}`;backdrop.classList.add('open');}
function closeResponseModal(){document.getElementById('response-modal-backdrop')?.classList.remove('open');}
async function viewLast(rid){if(!rid)return;openResponseModal(await fetchHistoryDetail(rid));}
async function loadHistory(){const h=await fetch('/history').then(r=>r.json());let html='<thead><tr><th>Request</th><th>Patient</th><th>Payer</th><th>Check Type</th><th>Status</th><th>Source</th><th>Created</th><th>Duration</th><th>Error</th><th>Actions</th></tr></thead><tbody>';for(const i of h.items){html+=`<tr><td title="${i.request_id||''}">${shortId(i.request_id)}</td><td>${i.patient}</td><td class='wrap-cell'>${i.payer}</td><td>${checkTypeLabel(i.service_type)}</td><td>${badge(i.status)}</td><td>${i.provider_source}</td><td>${i.created_at}</td><td>${i.duration_ms||''}</td><td class='wrap-cell'>${i.error_message||''}</td><td><div class='action-group'><button class='btn btn-secondary btn-sm history-view-btn' data-request-id='${i.request_id}'>View Details</button><button class='btn btn-primary btn-sm history-rerun-btn' data-request-id='${i.request_id}'>Rerun</button><button class='btn btn-secondary btn-sm history-edit-btn' data-request-id='${i.request_id}'>Edit Before Rerun</button></div></td></tr>`}document.getElementById('hist-table').innerHTML=html+'</tbody>';document.querySelectorAll('.history-view-btn').forEach((btn)=>{btn.addEventListener('click',()=>viewLast(btn.dataset.requestId||''));});document.querySelectorAll('.history-rerun-btn').forEach((btn)=>{btn.addEventListener('click',()=>rerun(btn.dataset.requestId||''));});document.querySelectorAll('.history-edit-btn').forEach((btn)=>{btn.addEventListener('click',()=>editBeforeRerun(btn.dataset.requestId||''));});}
...
async function loadAdminConfig(){const cfg=await fetch('/admin/runtime-config').then(r=>r.json());const val=(section,field)=>cfg?.[section]?.[field]?.value||'';document.getElementById('admin-eligibility-provider').value=val('global','eligibility_provider');document.getElementById('admin-database-url').value=val('global','database_url');document.getElementById('admin-stedi-api-key').value='';document.getElementById('admin-stedi-api-key').placeholder=val('stedi','api_key')||'Leave blank to keep current secret';document.getElementById('admin-stedi-base-url').value=val('stedi','base_url');document.getElementById('admin-stedi-provider-organization-name').value=val('stedi','provider_organization_name');document.getElementById('admin-availity-client-id').value=val('availity','client_id');document.getElementById('admin-availity-client-secret').value='';document.getElementById('admin-availity-client-secret').placeholder=val('availity','client_secret')||'Leave blank to keep current secret';document.getElementById('admin-availity-base-url').value=val('availity','base_url');document.getElementById('admin-availity-scope').value=val('availity','scope');document.getElementById('admin-availity-submitter-id').value=val('availity','submitter_id');document.getElementById('admin-availity-provider-npi').value=val('availity','provider_npi');document.getElementById('admin-availity-provider-tax-id').value=val('availity','provider_tax_id');document.getElementById('admin-status').textContent=JSON.stringify(cfg,null,2);}
async function saveAdminConfig(){const payload={global:{eligibility_provider:document.getElementById('admin-eligibility-provider').value},stedi:{base_url:document.getElementById('admin-stedi-base-url').value,provider_organization_name:document.getElementById('admin-stedi-provider-organization-name').value},availity:{client_id:document.getElementById('admin-availity-client-id').value,base_url:document.getElementById('admin-availity-base-url').value,scope:document.getElementById('admin-availity-scope').value,submitter_id:document.getElementById('admin-availity-submitter-id').value,provider_npi:document.getElementById('admin-availity-provider-npi').value,provider_tax_id:document.getElementById('admin-availity-provider-tax-id').value}};const stediSecret=document.getElementById('admin-stedi-api-key').value;const availitySecret=document.getElementById('admin-availity-client-secret').value;if(stediSecret){payload.stedi.api_key=stediSecret;}if(availitySecret){payload.availity.client_secret=availitySecret;}const r=await fetch('/admin/runtime-config',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const data=await r.json();document.getElementById('admin-status').textContent=JSON.stringify(data,null,2);await loadProviders();await loadManualProviders();await loadLiveDemoProviders();}
async function resetAdminConfig(){const r=await fetch('/admin/runtime-config/reset',{method:'POST'});document.getElementById('admin-status').textContent=JSON.stringify(await r.json(),null,2);await loadAdminConfig();await loadProviders();}
async function adminSetSource(){const provider=document.getElementById('admin-eligibility-provider').value;const resp=await fetch('/ui/select-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})});document.getElementById('admin-status').textContent=JSON.stringify(await resp.json(),null,2);}
...
async function renderFactorySourceStatus(status=null){const el=document.getElementById('factory-source-status');if(!el)return;el.textContent='Verification Source Status: loading...';const esc=(v)=>String(v).replace(/[&<>'"]/g,(c)=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));try{const [connections,details]=await Promise.all([fetch('/ui/connections').then(r=>r.json()),fetch('/ui/connection-details').then(r=>r.json())]);const provider=details.provider||connections.current_provider||status?.provider||'unknown';const configured=details.configured===true?'yes':details.configured===false?'no':'unknown';const connectionStatus=status?`${status.connected?'connected':'not connected'}${status.detail?` (${status.detail})`:''}`:'Click Set Source to refresh provider connection status.';const stediHint=provider==='stedi'&&details.configured?'API key configured. Send a demo Stedi row to verify payer connectivity.':'';const items=[['Selected provider',provider],['Configured',configured],['Base URL',details.base_url],['Supported transaction',details.supported_transaction],['Connection/source status',stediHint||connectionStatus],['Notes',details.notes],['Last checked',status?new Date().toLocaleString():'not checked this page load']];el.innerHTML=`<strong>Verification Source Status</strong><div class='metrics'>${items.filter(([_,v])=>v!==undefined&&v!==null&&v!=='').map(([label,value])=>`<div class='metric'><div class='label'>${esc(label)}</div><div class='value' style='font-size:.95rem'>${esc(value)}</div></div>`).join('')}</div>`;}catch(err){el.textContent=`Verification Source Status: unavailable (${err?.message||err})`;}}
async function setLiveDemoProvider(){const provider=document.getElementById('factory-provider').value;const resp=await fetch('/ui/select-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})});const status=await resp.json();addLiveDemoActivity(`POST /ui/select-connection → provider set to ${provider}`);await renderFactorySourceStatus(status);}
async function refreshLiveDemo(){const [w,h,o]=await Promise.all([fetch('/work-items').then(r=>r.json()),fetch('/history').then(r=>r.json()),fetch('/outbox').then(r=>r.json())]);liveDemoItems=w.items||[];liveDemoHistory=h.items||[];liveDemoOutbox=o.items||[];renderLiveDemoTable();renderLiveDemoMetrics();const refreshTime=document.getElementById('factory-refresh-time');if(refreshTime){refreshTime.textContent=`Refreshed at ${new Date().toLocaleString()}`;}}
...
async function validateLiveDemoAllPending(){const provider=document.getElementById('factory-provider').value;const pendingIds=liveDemoItems.filter(i=>i.needs_validation).map(i=>i.id);const r=await fetch('/work-items/validate-selected',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids:pendingIds,provider})}).then(x=>x.json());addLiveDemoActivity(`POST /work-items/validate-selected provider=${provider} → validated ${r.validated_count}, failed ${r.failed_count}`);await showLatestFactoryResult(r);await refreshLiveDemo();await loadWork();await loadHistory();await loadOutbox();await loadDemoCounts();await dashboard();}
async function loadProviders(status=null){const meta=await fetch('/ui/connections').then(r=>r.json());const sel=document.getElementById('provider');sel.innerHTML='';for(const p of meta.providers){const o=document.createElement('option');o.value=p;o.textContent=p;if(p===meta.current_provider)o.selected=true;sel.appendChild(o);}document.getElementById('provider-status').textContent=status?JSON.stringify(status,null,2):'Connection status not checked on page load. Click Switch Provider to refresh status.';const details=await fetch('/ui/connection-details').then(r=>r.json());document.getElementById('details').textContent=JSON.stringify(details,null,2);const matrix=await fetch('/ui/provider-matrix').then(r=>r.json());document.getElementById('adapter-matrix').textContent=JSON.stringify(matrix.providers,null,2);}
async function switchProvider(){const provider=document.getElementById('provider').value;const resp=await fetch('/ui/select-connection',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})});const data=await resp.json();await loadProviders(data);}
function getPayloadFromForm(){const payload={patient:{first_name:document.getElementById('first_name').value,last_name:document.getElementById('last_name').value,dob:document.getElementById('dob').value,member_id:document.getElementById('member_id').value},payer:{name:document.getElementById('payer_name').value,payer_id:document.getElementById('payer_id').value},provider:{npi:document.getElementById('npi').value,tax_id:document.getElementById('tax_id').value},service_type:document.getElementById('service_type').value};const externalPatientId=document.getElementById('external_patient_id').value.trim();if(externalPatientId){payload.external_patient_id=externalPatientId;}return payload;}
function populateFormFromPayload(payload){document.getElementById('first_name').value=payload?.patient?.first_name||'';document.getElementById('last_name').value=payload?.patient?.last_name||'';document.getElementById('dob').value=payload?.patient?.dob||'';document.getElementById('member_id').value=payload?.patient?.member_id||'';document.getElementById('external_patient_id').value=payload?.external_patient_id||'';document.getElementById('payer_name').value=payload?.payer?.name||'';document.getElementById('payer_id').value=payload?.payer?.payer_id||'';document.getElementById('npi').value=payload?.provider?.npi||'';document.getElementById('tax_id').value=payload?.provider?.tax_id||'';document.getElementById('service_type').value=payload?.service_type||'';}
...
async function viewFactoryResponse(workItemId){const item=liveDemoItems.find(x=>x.id===workItemId);const cached=factoryResponsesByWorkItemId[workItemId]||{};if(cached.historyDetail){openResponseModal(cached.historyDetail);return;}const requestId=cached.validationResponse?.request_id||item?.last_request_id;if(requestId){const detail=await fetchHistoryDetail(requestId);factoryResponsesByWorkItemId[workItemId]={...(factoryResponsesByWorkItemId[workItemId]||{}),historyDetail:detail,fetchedAt:new Date().toISOString()};openResponseModal(detail);return;}alert('No response has been captured for this row yet. Send the row first.');}
const defaultManualPayload={patient:{first_name:'Jane',last_name:'Doe',dob:'1985-06-15',member_id:'MBR123456'},payer:{name:'Blue Cross Blue Shield',payer_id:'BCBS001'},provider:{npi:'1234567890',tax_id:'12-3456789'},service_type:'30',external_patient_id:null};
(async()=>{document.getElementById('payload').value=JSON.stringify(defaultManualPayload,null,2);populateFormFromPayload(defaultManualPayload);setManualMode('form');showTab('dashboard');addLiveDemoActivity('Live demo ready.');await safeCall('dashboard',()=>dashboard());await safeCall('loadWork',()=>loadWork());await safeCall('loadHistory',()=>loadHistory());await safeCall('loadOutbox',()=>loadOutbox());await safeCall('loadDemoCounts',()=>loadDemoCounts());await safeCall('loadProviders',()=>loadProviders());await safeCall('loadAdminConfig',()=>loadAdminConfig());await safeCall('loadManualProviders',()=>loadManualProviders());await safeCall('loadLiveDemoProviders',()=>loadLiveDemoProviders());await safeCall('refreshLiveDemo',()=>refreshLiveDemo());})();
</script></body></html>"""



@app.get("/admin/runtime-config")
async def admin_runtime_config() -> dict:
    return runtime_config.response()


@app.patch("/admin/runtime-config")
async def admin_patch_runtime_config(payload: dict) -> dict:
    try:
        runtime_config.patch(payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    provider = runtime_config.get_effective_value("global", "eligibility_provider").lower()
    if provider != service.get_provider() and provider in get_available_connections():
        service.set_provider(provider)
    return runtime_config.response()


@app.post("/admin/runtime-config/reset")
async def admin_reset_runtime_config() -> dict:
    runtime_config.reset()
    provider = runtime_config.get_effective_value("global", "eligibility_provider").lower()
    if provider in get_available_connections() and provider != service.get_provider():
        service.set_provider(provider)
    return runtime_config.response()
...
async def history_item(request_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        "result": None if not res else {
            ...
            "raw_response_json": parse_stored_json(res.raw_response_json),
            "notes": res.notes,
        },
        "outbox": None if not outbox else {"id": outbox.id, "status": outbox.status, "target_system": outbox.target_system, "target_record_type": outbox.target_record_type, "payload_json": parse_stored_json(outbox.payload_json)},
    }
...
async def _validate_work_item(item: VerificationWorkItem, db: Session, fallback_provider: str | None = None) -> dict:
    ...
    try:
        ...
        response = await service.check(payload)
        complete_request_success(db, req_rec, response, started)
        item.validation_status, item.needs_validation, item.last_error_message = work_item_status_from_response(response)
        item.last_validated_at = datetime.utcnow() if item.validation_status == "validated" else None
        item.last_request_id = req_rec.request_id
        db.commit()
        db.refresh(item)
        return {"id": item.id, "validation_status": item.validation_status, "status": item.validation_status, "needs_validation": item.needs_validation, "last_request_id": item.last_request_id, "request_id": item.last_request_id, "source_system": item.source_system, "source_file_name": item.source_file_name, "source_row_number": item.source_row_number, "notes": item.notes, "manual_override_reason": item.manual_override_reason, "last_error_message": item.last_error_message, "last_validated_at": item.last_validated_at.isoformat() if item.last_validated_at else None, "created_at": item.created_at.isoformat(), "updated_at": item.updated_at.isoformat(), "is_demo": item.is_demo, "preferred_provider": item.preferred_provider, "provider": selected_provider, "first_name": item.first_name, "member_id": item.member_id}
    except Exception as exc:
        error_message = str(exc)
        if req_rec is not None:
            complete_request_error(db, req_rec, error_message, started)
    ...
    for item_id in ids:
        ...
        try:
            ...
            if result.get("validation_status") == "validated":
                ...
                failed_count += 1
            results.append({"id": item.id, "status": result.get("validation_status"), "request_id": result.get("last_request_id"), "provider": result.get("provider"), "first_name": item.first_name, "member_id": item.member_id, "last_error_message": result.get("last_error_message")})
...
@app.get("/exports/factory/results.csv", response_class=PlainTextResponse)
async def factory_results_export(include_demo: bool = True, db: Session = Depends(get_db)) -> str:
    q = db.query(VerificationWorkItem)
    if not include_demo:
        q = q.filter(VerificationWorkItem.is_demo.is_(False))
    items = q.order_by(VerificationWorkItem.created_at.desc()).all()
    ...
    w.writeheader()
    for item in items:
        request_id = item.last_request_id
        req = None
        res = None
        outbox = None
        if request_id:
        ...
        request_payload = {
            "patient": {
                ...
                "dob": item.dob.isoformat() if item.dob else None,
        ...
        w.writerow({
            ...
            "dob": item.dob.isoformat() if item.dob else "",
            "member_id": item.member_id,
            "payer_name": item.payer_name,
            "payer_id": item.payer_id,
            "provider_source": req.provider_source if req else "",
            "validation_status": item.validation_status,
            "request_id": request_id or "",
            "request_payload_json": json.dumps(request_payload, sort_keys=True),
            "response_payload_json": res.raw_response_json if res else "",
            "authorization_required": res.authorization_required if res else "",
            "plan_name": res.plan_name if res else "",
            "copay": res.copay if res else "",
            "coinsurance": res.coinsurance if res else "",
            "deductible_remaining": res.deductible_remaining if res else "",
            "out_of_pocket_remaining": res.out_of_pocket_remaining if res else "",
            "outbox_status": outbox.status if outbox else "",
            "last_validated_at": item.last_validated_at.isoformat() if item.last_validated_at else "",
        })
    return out.getvalue()
