...
async function refreshLiveDemo(){const [w,h,o]=await Promise.all([fetch('/work-items').then(r=>r.json()),fetch('/history').then(r=>r.json()),fetch('/outbox').then(r=>r.json())]);liveDemoItems=w.items||[];liveDemoHistory=h.items||[];liveDemoOutbox=o.items||[];renderLiveDemoTable();renderLiveDemoMetrics();await renderFactorySourceStatus();}
